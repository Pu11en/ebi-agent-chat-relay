...
Run the standalone coordinator with a separate state directory per run. Its
default queue admission submits approved builders even while other projects
are busy; `--max-running` bounds this run's outstanding workers. It
