...
- [x] 4.4 **Depends on 4.2; owner: Setup Agent handoff** — Add Ask Setup Agent using the existing session creation contract and a bounded safe item packet; verify tests create one management thread, attach no raw secret content, and perform no edit before the user's request is processed normally.
