...
Goal: The settings button reads "Save changes" and everything else stays as it is
...
- The label reads "Save changes", not "Save" (changed while the build ran)
...
```gowork-plan
{
  "schema_version": 1,
  "plans": [
    {"id": "settings", "version": 2, "project_path": "."},
  ...
  "tasks": [
    {
      ...
      "owned_resources": [],
      "required_inputs": ["REQ-SAVE-LABEL: the label reads Save changes"],
      "output": "The button label reads Save changes and its snapshot test passes",
