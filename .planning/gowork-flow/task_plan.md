...
- [ ] **T15: Finish automatically without looks-good approval.** Automatically integrate each checked build into its local project, serialize same-project integration and check the combined result; close workers, retain results and remove success waiters/reminders. Needs: T14a-T14c and retention decision. Proof: success needs no user response, existing edits survive, concurrent builds cannot race integration, blockers remain actionable through direct message replies, and a failed integration cannot destroy work or masquerade as completion.
