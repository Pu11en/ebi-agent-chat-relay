...
[![CI](https://github.com/Pu11en/ebi-agent-chat-relay/actions/workflows/ci.yml/badge.svg)](https://github.com/Pu11en/ebi-agent-chat-relay/actions/workflows/ci.yml)
[![CodeQL](https://github.com/Pu11en/ebi-agent-chat-relay/actions/workflows/codeql.yml/badge.svg)](https://github.com/Pu11en/ebi-agent-chat-relay/actions/workflows/codeql.yml)
...
- **Conflict-aware worktree instructions** — A session works directly in its assigned project unless another active session uses that same project. Only the colliding session is told to create `.worktrees/wt-{thread_id}` inside the owning project. Existing session worktrees are always resumed.
- **Automatic worktree cleanup** — Requires `WORKTREE_BASE_DIR`. When set, legacy top-level and project-local session worktrees are inspected at session end and bot startup. Cleanup preserves tracked, untracked, and ignored local files; only an empty checkout is removed automatically.
...
uvx --from "git+https://github.com/Pu11en/ebi-agent-chat-relay.git" ccdb setup
...
```bash
uv add git+https://github.com/Pu11en/ebi-agent-chat-relay.git
```
...
| `WORKTREE_BASE_DIR` | Parent directory of your repositories, scanned for legacy and project-local session worktrees. Recommended when concurrent sessions may edit the same project. | (optional) |
